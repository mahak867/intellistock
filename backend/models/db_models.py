...
class User(Base, TimestampMixin):
    ...
    portfolio_entries = relationship(
        "PortfolioEntry", back_populates="user", cascade="all, delete-orphan"
    )
...
class PredictionLog(Base, TimestampMixin):
    ...
    __table_args__ = (Index("ix_predlog_ticker_created", "ticker", "created_at"),)
...
class ModelVersion(Base, TimestampMixin):
    ...
    )  # only 1 active per ticker
    trained_by = Column(String(100), nullable=True)  # "auto" or user email

    __table_args__ = (Index("ix_modelver_ticker_active", "ticker", "is_active"),)
...
class AlertRule(Base, TimestampMixin):
    ...
    )  # "signal" | "price_above" | "price_below"
    signal_filter = Column(
        String(10), nullable=True
    )  # BUY | SELL | HOLD (for signal alerts)
